...
// Posts to /api/corporate-lead; WhatsApp offered as the zero-friction fallback.
export function CorporateLeadForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/corporate-lead", {
        ...
        body: JSON.stringify({ name, company, phone, quantity, message }),
      });
      if (res.ok) {
  ...
  const waMessage =
    ...
    (company.trim() ? ` I'm from ${company.trim()}.` : "") +
    (quantity.trim() ? ` We're looking at roughly ${quantity.trim()} pieces.` : "");
  ...
  if (status === "success") {
    return (
      <div style={{ maxWidth: "480px", margin: "0 auto", textAlign: "center" }}>
        ...
        >
          Parul will reach out within one business day with concepts and pricing.
          In a hurry? Message us directly on WhatsApp.
        </p>
        <a
    ...
    <form onSubmit={submit} style={{ maxWidth: "520px", margin: "0 auto", textAlign: "left" }}>
      ...
      {status === "error" && (
        <p style={{ fontFamily: "var(--font-geist-sans, 'Inter', sans-serif)", fontSize: "13px", color: "#f0a68c", marginBottom: "14px" }}>
          Couldn&apos;t submit — check name &amp; phone, or just message us on WhatsApp below.
        </p>
      )}
